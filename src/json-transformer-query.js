...
function slice (obj, start, end, step) {	// eslint-disable-line no-unused-vars
...
function expand (obj, name) {		// eslint-disable-line no-unused-vars
  ...
  function walk (obj) {
    ...
    for (const i in obj) {	// eslint-disable-line no-restricted-syntax, guard-for-in
      const val = obj[i];
      if (!name) {
...
function distinctFilter (array, callback) {		// eslint-disable-line no-unused-vars
  ...
  for (let i = 0, l = outArr.length; i < l; ++i) {
...
function mapFilter (array, callback) {			// eslint-disable-line no-unused-vars
  return array.map(callback);
}

function filterFilter (array, callback) {		// eslint-disable-line no-unused-vars
  return array.filter(callback);
}

const jsonQuery = function jsonQuery (query, obj) {
  // summary:
  ...
  function makeRegex (t, a, b, c, d, e, f, g) {
    ...
    return str[g].match(/[\*\?]/) || f === '~' ?
      '/^' + str[g].substring(1, str[g].length - 1).replace(/\\([btnfr\\"'])|([^\w\*\?])/g, '\\$1$2').replace(/([\*\?])/g, '[\\w\\W]$1') + (f === '~' ? '$/i' : '$/') + `.test(${a})` :		// eslint-disable-line prefer-template
      t;
  }
  ...
  query = query.replace(/([^=]=)([^=])/g, '$1=$2').replace(/@|(\.\s*)?[a-zA-Z\$_]+(\s*:)?/g, (t) => (
    t.charAt(0) === '.' ? t : 	// leave .prop alone
  ...
  .replace(/\.?\.?\[(`\]|[^\]])*\]|\?.*|\.\.([\w\$_]+)|\.\*/g, (t, a, b) => {
    let oper = t.match(/^\.?\.?(\[\s*\^?\?|\^?\?|\[\s*==)(.*?)\]?$/); // [?expr] and ?expr and [=expr and =expr
    if (oper) {
      let prefix = ''; // eslint-disable-line no-shadow
      if (t.match(/^\./)) {
        // recursive object search
        call('expand');
        prefix = ',true)';
    ...
    if (oper) {
      ...
      return '.concat().sort(function(a,b){' + oper[1].replace(/\s*,?\s*([\/\\])\s*([^,\\\/]+)/g, (t, a, b) => {	// eslint-disable-line prefer-template
  ...
  .replace(/`([0-9]+)\s*(==|~)\s*(\$obj\s*((\.\s*[\w_$]+)|(\[\s*`([0-9]+)\s*`\]))*)/g, (t, a, b, c, d, e, f, g) => makeRegex(t, c, d, e, f, g, b, a)); // and do it for reverse =

  query = prefix + (query.charAt(0) === '$' ? '' : '$') + query.replace(/`([0-9]+|\])/g, (t, a) => (a === ']' ? ']' : str[a])); // restore the strings

  // create a function within this scope (so it can use expand and slice)
  const executor = eval(`1&&function($,$1,$2,$3,$4,$5,$6,$7,$8,$9){var $obj=$;return ${query}}`);
